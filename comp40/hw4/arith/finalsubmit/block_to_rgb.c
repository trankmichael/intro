...
{ 
        ...
        image->pixels = image->methods->new(image->width, image->height, 
                                                    sizeof(struct Pnm_rgb));
        image->methods->map_row_major( block_array, &apply_rgb_to_block, 
                                                                   &image );
...
{  
        ...
        red = force_float( pix.red * (float)DENOMINATOR,
                                         (float) DENOMINATOR, 0 );
        green = force_float( pix.green * (float)DENOMINATOR,
                                         (float) DENOMINATOR, 0 );
        blue = force_float( pix.blue * (float)DENOMINATOR,
                                         (float) DENOMINATOR, 0 );
